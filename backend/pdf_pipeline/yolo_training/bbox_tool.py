"""bbox 좌표 측정 도구 — 44장 연속 라벨링

사용법:
  python bbox_tool.py

조작:
  - 왼쪽 클릭 2번: bbox 지정 (1클릭=좌상단, 2클릭=우하단)
  - u: 마지막 박스 취소
  - n / Enter / Space: 다음 이미지 (현재 페이지 저장 후 이동)
  - b: 이전 이미지로 돌아가기
  - q: 종료 + 전체 결과를 labels_output.py로 저장
"""
import cv2
import numpy as np
from pathlib import Path

IMAGES_DIR = Path("dataset/images/train")
LABELS_DIR = Path("dataset/labels/train")
SCALE = 0.35  # 표시 비율

# 전체 이미지 순서
ALL_PAGES = (
    [f"exam_2021_page_{i:02d}" for i in range(1, 13)] +
    [f"exam_2026_page_{i:02d}" for i in range(1, 13)] +
    [f"exam_odd_page_{i:02d}"  for i in range(1, 21)]
)

W, H = 3509, 4963

# 전역 상태
state = {
    "boxes": [],    # (num, x1, y1, x2, y2) 원본 좌표
    "clicks": [],   # 현재 진행 중인 클릭 (표시 좌표)
    "num": 1,
    "dirty": False,
}
all_results = {}    # page_name → [(num, x1, y1, x2, y2), ...]
img_orig = None


def scale_to_orig(x, y):
    return int(round(x / SCALE)), int(round(y / SCALE))


def render():
    out = cv2.resize(img_orig, None, fx=SCALE, fy=SCALE)
    for num, x1, y1, x2, y2 in state["boxes"]:
        sx1, sy1 = int(x1 * SCALE), int(y1 * SCALE)
        sx2, sy2 = int(x2 * SCALE), int(y2 * SCALE)
        cv2.rectangle(out, (sx1, sy1), (sx2, sy2), (0, 0, 255), 2)
        cv2.putText(out, str(num), (sx1 + 4, sy1 + 26),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 0, 255), 2)
    if len(state["clicks"]) == 1:
        cv2.circle(out, state["clicks"][0], 6, (0, 255, 0), -1)
    return out


def mouse_cb(event, x, y, flags, param):
    if event != cv2.EVENT_LBUTTONDOWN:
        return
    state["clicks"].append((x, y))
    if len(state["clicks"]) == 2:
        (sx1, sy1), (sx2, sy2) = state["clicks"]
        sx1, sx2 = min(sx1, sx2), max(sx1, sx2)
        sy1, sy2 = min(sy1, sy2), max(sy1, sy2)
        ox1, oy1 = scale_to_orig(sx1, sy1)
        ox2, oy2 = scale_to_orig(sx2, sy2)
        num = state["num"]
        state["boxes"].append((num, ox1, oy1, ox2, oy2))
        state["num"] += 1
        state["clicks"] = []
        state["dirty"] = True
        print(f"  [{num}번] ({ox1},{oy1}) → ({ox2},{oy2})")
    cv2.imshow("bbox_tool", render())


def save_yolo(page_name, boxes):
    """YOLO 형식 라벨 즉시 저장"""
    LABELS_DIR.mkdir(parents=True, exist_ok=True)
    lines = []
    for _, x1, y1, x2, y2 in boxes:
        cx = ((x1 + x2) / 2) / W
        cy = ((y1 + y2) / 2) / H
        w  = (x2 - x1) / W
        h  = (y2 - y1) / H
        lines.append(f"0 {cx:.6f} {cy:.6f} {w:.6f} {h:.6f}")
    path = LABELS_DIR / f"{page_name}.txt"
    path.write_text("\n".join(lines))


def save_output():
    """labels_output.py 에 전체 결과 기록"""
    lines = ["# 자동 생성된 라벨 좌표\nPAGE_LABELS = {\n"]
    for pname, blist in all_results.items():
        lines.append(f'  "{pname}": [\n')
        for num, x1, y1, x2, y2 in blist:
            lines.append(f'    ({num}, {x1}, {y1}, {x2}, {y2}),\n')
        lines.append('  ],\n')
    lines.append("}\n")
    Path("labels_output.py").write_text("".join(lines), encoding="utf-8")
    print("\n[저장] labels_output.py 저장 완료")


def main():
    global img_orig

    cv2.namedWindow("bbox_tool", cv2.WINDOW_NORMAL)
    cv2.setMouseCallback("bbox_tool", mouse_cb)

    idx = 0
    total = len(ALL_PAGES)

    while idx < total:
        page_name = ALL_PAGES[idx]
        img_path = IMAGES_DIR / f"{page_name}.png"

        if not img_path.exists():
            print(f"[SKIP] {page_name} 이미지 없음")
            idx += 1
            continue

        # 기존 결과가 있으면 복원 (b로 돌아왔을 때)
        state["boxes"] = list(all_results.get(page_name, []))
        state["clicks"] = []
        state["num"] = len(state["boxes"]) + 1
        state["dirty"] = False

        img_orig = cv2.imread(str(img_path))
        h_disp = int(img_orig.shape[0] * SCALE)
        w_disp = int(img_orig.shape[1] * SCALE)
        cv2.resizeWindow("bbox_tool", w_disp, h_disp)

        print(f"\n[{idx+1}/{total}] {page_name}  (박스 {len(state['boxes'])}개 로드)")
        cv2.setWindowTitle("bbox_tool",
            f"[{idx+1}/{total}] {page_name}  |  u=취소  n/Enter=다음  b=이전  q=종료")

        action = None
        while action is None:
            cv2.imshow("bbox_tool", render())
            key = cv2.waitKey(20) & 0xFF
            if key in (ord('n'), ord(' '), 13):   # n / Space / Enter
                action = 'next'
            elif key == ord('b'):
                action = 'prev'
            elif key == ord('q'):
                action = 'quit'
            elif key == ord('u'):
                if state["boxes"]:
                    removed = state["boxes"].pop()
                    state["num"] -= 1
                    state["clicks"] = []
                    print(f"  [취소] {removed[0]}번 박스 삭제")
                    cv2.imshow("bbox_tool", render())

        # 현재 페이지 저장
        if state["boxes"]:
            all_results[page_name] = list(state["boxes"])
            save_yolo(page_name, state["boxes"])
            print(f"  → {len(state['boxes'])}개 저장")

        if action == 'next':
            idx += 1
        elif action == 'prev':
            idx = max(0, idx - 1)
        elif action == 'quit':
            break

    cv2.destroyAllWindows()
    save_output()
    print(f"\n완료! 총 {len(all_results)}개 페이지 라벨링됨")
    print("YOLO 라벨: dataset/labels/train/")
    print("좌표 백업: labels_output.py")


if __name__ == "__main__":
    main()

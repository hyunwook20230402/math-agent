...
## 현재 DB 상태
- RLS: **비활성화** (마이그레이션 `20250116000003`)
...
solution_nodes        — 막힌 지점 도우미 RAG 코퍼스 (problem_id → problems.id, 풀이 step 단위 추론 노드 + bge-m3 1024 embedding + uses INT[]/whys JSONB = 전이 DAG·논리 근거. 마이그레이션 012)
```

...
2. **VL 호출 통일** — VL 은 항상 `vl_providers.call_vl()` 경유(OpenAI 단일). 직접 OpenAI API 호출 금지.
...
8. **VL 키 확인** — `OPENAI_API_KEY` 필요(VL=OpenAI 단일). 임베딩용 ollama bge-m3 는 `ollama list` 에 `bge-m3` 있어야 함.
